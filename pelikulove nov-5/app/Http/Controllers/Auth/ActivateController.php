...
class ActivateController extends Controller
{
     ...
     * @return Redirect
     */
    // verification page of ricky lee users
    public function initialRickyLee()
    {
        $user = Auth::user();
        $lastActivation = Activation::where('user_id', $user->id)->get()->last();
        $currentRoute = Route::currentRouteName();
        ...
        return view('auth.activation-rickylee');
    }
    ...
    public function activationRequired()
    {
        ...
        $rlRegisterLog = RegisterLog::where('user_id', $user->id)
        ->where('event', 'RickyLeeLandingPage')
        ->first();

        if (isset($rlRegisterLog)) {
            // return redirect('forum/23-ricky-lee-talks-webinars/33-kung-paano-ko-binuo-ang-aking-sarili-sa-pagsusulat-at-pagtuturo-tambayan-with-ricky-lee')
            // ->with('status', 'success')
