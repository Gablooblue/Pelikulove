...
class ProfilesController extends Controller
{
     ...
     * @return mixed
     */
    public function edit($username, $focus = 'profile')
    {
        try {
            $user = $this->getUserByUsername($username);
        } catch (ModelNotFoundException $exception) {
        ...
        return view('profiles.edit', compact('focus'))->with($data);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function updateNotifSettings(Request $request, $id)
    {
        $currentUser = Auth::user();
        $user = User::findOrFail($id);

        if ($currentUser == $user) {              
            if ($request->input('tambayanChkbx') == "on") {
                $tambayanChkbx = 1;
            } else {
                $tambayanChkbx = 0;
            }            

            if ($request->input('saluhanChkbx') == "on") {
                $saluhanChkbx = 1;
            } else {
                $saluhanChkbx = 0;
            }       

            if ($request->input('topicChkbx') == "on") {
                $topicChkbx = 1;
            } else {
                $topicChkbx = 0;
            } 

            if ($request->input('blackboxVidChkbx') == "on") {
                $blackboxVidChkbx = 1;
            } else {
                $blackboxVidChkbx = 0;
            }
            
            $notifPref = NotifPreferences::where('user_id', $user->id)
            ->first();
            
            if (isset($notifPref)) {               
                $notifPref->tambayan_email = $tambayanChkbx;
                $notifPref->saluhan_email = $saluhanChkbx; 
                $notifPref->topic_email = $topicChkbx; 
                $notifPref->vod_email = $blackboxVidChkbx; 
                $notifPref->save();                  
            } else {
                $newNotifPref = NotifPreferences::create([
                    'user_id'               => $user->id,
                    'tambayan_email'        => $tambayanChkbx,
                    'saluhan_email'         => $saluhanChkbx,
                    'topic_email'           => $topicChkbx,
                    'vod_email'             => $blackboxVidChkbx,
                ]);
                $newNotifPref->save();    
            } 
            return redirect('profile/'.$user->name.'/edit')->with('success', 'Your Notification Preferences has been updated.'); 
        } else {
            return redirect('profile/'.$user->name.'/edit')->with('warning', 'Please try again.');
        }
        // dd($request);
    }
