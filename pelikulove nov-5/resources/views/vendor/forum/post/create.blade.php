...
@section ('content')
    <div id="create-post">
        <h4>{{ trans('forum::general.new_reply') }} ({{ $thread->title }})</h4>
        ...
        <form method="POST" action="{{ Forum::route('post.store', $thread) }}">
            ...
            <button type="submit" class="btn btn-info pull-right">{{ trans('forum::general.reply') }}</button>
            <a href="{{ URL::previous() }}" class="btn btn-link text-info pull-right">{{ trans('forum::general.cancel') }}</a>
        </form>
    </div>
@stop
