@foreach($comments as $comment)
    <div class="display-comment" @if($comment->parent_id != null) style="margin-left:40px;" @endif>
        	<div class="media mt-1 mb-2"><a class="text-dark" href="{{url('/profile/'.$comment->user->name)}}">
        		<img src="@if ($comment->user->profile->avatar_status == 1) {{ $comment->user->profile->avatar }} @else {{ Gravatar::get($comment->user->email) }} @endif" alt="{{ $comment->user->name }}" class="user2-avatar-nav mt-1"></a>
  				...
  				<div class="media-body mt-0 pt-0">
     				<div class="font-weight-bold my-0 py-0"> <a class="text-dark" href="{{url('/profile/'.$comment->user->name)}}">{{ $comment->user->name }}</a> <span class="pl-2 small font-italic"> {{\Carbon\Carbon::createFromTimeStamp(strtotime($comment->created_at))->diffForHumans()}}</span></div>
