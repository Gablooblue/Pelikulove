@foreach($comments as $comment)
    <div class="display-comment" @if($comment->parent_id != null) style="margin-left:40px;" @endif>
        <div class="comment @if($comment->parent_id == null) border-bottom border-light @endif">
        		...
        		@if ($level+1 < 2)<a class="btn btn-sm btn-primary float-right mt-4" data-toggle="collapse" href="#rcommentModal{{$comment->id}}" role="button" aria-expanded="false" aria-controls="rcommentModal{{$comment->id}}"><i class="fa fa-reply"></i> Reply</a>@endif

        		<p>{{ $comment->body }} </p>
        		
        		@if ($level+1 < 2)
        		<div class="collapse multi-collapse" id="rcommentModal{{$comment->id}}">
        			<form method="post" class="rcommentForm" id="rcommentForm_{{ $comment->id }}"> 
        		 @csrf
            		...
            		<div class="form-group">
                		<input type="submit" class="btn btn-sm btn-success" data-id="{{$comment->id}}" value="Reply" />
        			...
        			@include('lessons.commentsDisplay2', ['comments' => $comment->replies, 'level' => $level+1])
        		@endif	
